...
from .openai_helper import build_system_prompt, get_chatbot_response, detect_query_type, get_user_memory
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    user_id = websocket.query_params.get("userId"''', f"guest-{str(uuid.uuid4())}"''')  # 참고용
        ...
        while True:
            ...
            if response_parts:
                ...
                if query_type == "개인기록검색":
                    ...
                    try:
                        print("📤 GPT 요청 history:", pk, msg[:30], system_prompt[:30])
                        reply = await get_chatbot_response(pk, msg, system_prompt)
